...
subroutine update(n, A, b, x)
    ...
    integer :: i, j
    real, dimension(n) :: x_temp

    do i = 1, n
        x_temp(i) = b(i)
        do j = 1, n
            if (i .ne. j) x_temp(i) = x_temp(i) - a(i, j) * x(j)
        end do
        x_temp(i) = x_temp(i) / a(i, i)
    end do
    x = x_temp
end subroutine update


subroutine gaussJacobi(n, A, b, x, tolerance, max_iterations)
    implicit none
...
program mainProgram
    ...
    call gaussJacobi(n, A, b, x, tolerance, 100)
